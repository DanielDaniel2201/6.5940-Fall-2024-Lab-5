    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i w_lower_half_next = _mm256_and_si256(lowMask, raw_w_next);
                ...
                __m256i w_shifted_next = _mm256_srli_epi16(raw_w_next, 4);
                __m256i w_higher_half_next = _mm256_and_si256(w_shifted_next, lowMask);
                // TODO: apply zero_point to weights and convert the range from (0, 15) to (-8, 7)
                ...
                w_0_next = _mm256_sub_epi8(w_lower_half_next, zero_point);
                w_128_next = _mm256_sub_epi8(w_higher_half_next, zero_point);
